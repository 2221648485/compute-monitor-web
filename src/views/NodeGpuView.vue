...
async function load() {
  ...
  try {
    ...
    try {
      // GPU 遥测依赖 Prometheus/DCGM，失败时只提示，不阻断节点清单展示。
      const gpuList = await monitorApi.gpus(props.clusterId);
      gpus.value = Array.isArray(gpuList) ? gpuList : [];
    } catch (err) {
      toast.warning(err instanceof Error ? err.message : 'GPU 遥测暂不可用');
    }
  } catch (err) {
    toast.error(err instanceof Error ? err.message : '加载节点资源失败');
  } finally {
    loading.value = false;
  }
}
  ...
  <section class="view-stack">
    ...
    <template v-if="clusterId">
      <div class="panel-grid two">
        <article class="panel fixed-panel">
          <div class="panel-head">
            <span class="eyebrow">Nodes</span>
            <h2>节点状态</h2>
          </div>
          <div v-if="nodes.length" class="node-list scroll-list node-scroll">
        ...
        <article class="panel fixed-panel">
          ...
          <div v-if="gpus.length" class="load-list scroll-list gpu-scroll">
            <div v-for="gpu in gpus" :key="`${gpu.nodeName}-${gpu.gpuIndex}`" class="gpu-row">
      ...
      <article class="panel">
        ...
        <div v-if="nodes.length" class="scroll-table inventory-table">
          <DataTable
